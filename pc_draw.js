...
var time=0;
    ...
    function stopLottery() {
        ...
        // 修改为指定人员，一般从后台随机抽取,没有则前端随机抽取 lcf
        if (backChoose) {
            $('#lottery-wrap .lottery-list').eq(sure_index)
            .attr('data-namezh', backChoose).attr('data-nameen', 'avatar-backend')
            .find('.content-detail>span').html(backChoose).parent().siblings('.content-title')
            .children('.lottery-name').html(backChooseName)
        }
...
//		context.drawImage(back_img, 0, 0);
	...
	//		=name_zh.substring(0,1)+" "+name_zh.substring(1,2)+" "+name_zh.substring(2,3)+" "+name_zh.substring(3,4);
			context.fillText(name_str, 75, 428);
...
//                  this.music.pause();
		...
		var lottery_btn_fun=function(){
            ...
            if (!isStart && isMove) {
                startLottery();
                $.ajax({
                    url: 'http://item.xianghunet.com/index/koi/luck',
                    type: 'get',
                    success (res) {
                        backChoose = res.res.tel
                        backChooseName = res.res.nickname
                    },
                    failure () {
                        backChoose = '12345678901'
                        backChooseName = '钱琛'
                    }
                })
            } else if(isStart) {
